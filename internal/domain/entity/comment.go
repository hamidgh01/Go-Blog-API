...
type Comment struct {
	...
	PostParentID    sql.Null[uint64] // sql: BIGINT REFERENCES posts(id) ON DELETE CASCADE (indexed WHERE postParentID IS NOT NULL;)
	CommentParentID sql.Null[uint64] // sql: BIGINT REFERENCES comments(id) ON DELETE CASCADE (indexed WHERE commentParentID IS NOT NULL;)
	// FK:
	User          *User
	PostParent    *Post
